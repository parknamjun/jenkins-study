...
* ansible server k8s/k8s-cicd-deployment-playbook.yml
````
...
````
* ansible server k8s/k8s-cicd-service-playbook.yml
````
...
* [root@ansible k8s]# ansible-playbook -i hosts k8s-cicd-service-playbook.yml -u parknamjun
* kubectl로 확인
...
* http://localhost:32000/hello-world/ 확인

---
## 실습7) Jenkins + Ansible + Kubernetes 연동하기(1)




## 실습7) Jenkins + Ansible + Kubernetes 연동하기(2)
...
* ansible에서 k8s로 ssh 접속을 할 수 있어야 함, Mac은 설정에서 원격로그인이 가능하게 설정하면 ssh로 원격에서 접속가능하다.
* 
